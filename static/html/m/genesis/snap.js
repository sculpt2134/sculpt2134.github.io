...
(function(win, doc) {
    ...
    var Snap = Snap || function(userOpts) {
                ...
                get: {
                    matrix: function(index) {
                        ...
                        if( !utils.canTransform() ){
                            return parseInt(settings.element.style.top, 10);
                        } else {
            ...
            drag: {
                ...
                dragging: function(e) {
                    if (cache.isDragging && settings.touchToDrag) {
                        ...
                        var thePageX = utils.page('X', e),
                            ...
                            openingTop = absoluteTranslation > 0,
                        ...
                        if (cache.hasIntent === false || cache.hasIntent === null) {
                            var deg = utils.angleOfDrag(thePageX, thePageY),
                                inBottomRange = (deg >= 0 && deg <= settings.slideIntent) || (deg <= 360 && deg > (360 - settings.slideIntent)),
                                inTopRange = (deg >= 180 && deg <= (180 + settings.slideIntent)) || (deg <= 180 && deg >= (180 - settings.slideIntent));
                            if (!inTopRange && !inBottomRange) {
                ...
                endDrag: function(e) {
                    if (cache.isDragging) {
                        ...
                        // Revealing top
                        if (cache.simpleStates.opening === 'top') {
                            ...
                            // Revealing Right
                        } else if (cache.simpleStates.opening === 'bottom') {
                            // Halfway, Flicking, or Too Far Out
                            if ((cache.simpleStates.halfway || cache.simpleStates.hyperExtending || cache.simpleStates.flick)) {
        ...
        this.state = function() {
            var state,
                fromTop = action.translate.get.matrix(4);
            if (fromTop === settings.maxPosition) {
                state = 'top';
            } else if (fromTop === settings.minPosition) {
                state = 'bottom';
            } else {
                state = 'closed';
            }
