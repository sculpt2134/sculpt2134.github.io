...
var Tran = {
    ...
    SetDataStep: function (data) {
        ...
        // ajax url 설정
        if (data == "step2") {
            this.url = '/sec/xhr/bespoke/bespokeB2CProductTypeInfo';
        } else if (data == "step3") {
            this.url = '/sec/xhr/bespoke/bespokeB2CModelInfo';
        } else if (data == "step4") {
            this.url = '/sec/xhr/bespoke/bespokeB2CColorMapInfo';
        } else if (data == "step4By360") {
            this.url = '/sec/xhr/bespoke/bespokeB2C360ColorMapInfo';
        } else if (data == "stepByPanel") {
            this.url = '/sec/xhr/bespoke/bespokePanelMapInfo';
        } else if (data == "modelAdd") {
            this.url = '/sec/xhr/bespoke/bespokeMyModelAdd';
        } else if (data == "myModelInfo") {
            this.url = '/sec/xhr/bespoke/myModelInfo';
        } else if (data == "DeleteSave") {
            this.url = '/sec/xhr/bespoke/bespokeDeleteCustomInfo';
        } else if (data == 'existsGoodsBase') {
            this.url = '/sec/xhr/bespoke/bespokeExistsGoodsBase';
        }
    },
    // ajax dataType 설정
...
var Cooler = {
        ...
        if (filteredData.length > 0) {
            for (var i = 0; i < filteredData.length; i++) {
                ...
                if (selectDepartment != 'F/P') {
                    ...
                    html += "<li class='list-item' onclick='Cooler.SetChangePanelColorInfo(this.dataset);' id='ref" + obj.typeSeq + "' data-id='ref" + obj.typeSeq + "' data-pkgType='" + obj.pkgType + "' data-imgUrl='" + obj.imgUrl + "' data-doorCnt='" + obj.doorCnt + "' data-upDep='" + selectDepartment + "' data-dep='" + obj.department + "' data-typeSeq='" + obj.typeSeq + "' data-type='" + obj.dataType + "' data-doorDiv='" + obj.doorDiv + "'  data-cbmGb='" + obj.cmbGb + "' data-module='" + obj.modelName + "' data-goods-id='" + obj.goodsId + "' data-typeName='" + obj.typeName + "'>";
                }
    ...
    SetSimulatorREFHTML: function (thumb) {
        ...
        htmlsource += '<img id="img_cooler_' + this.refIndex + '" src="https:' + this.getImgSrcThumb(imgURL, type, moduleColor) + '" alt="' + thumb.typename + '">';			// 이미지 경로 지정 할 필요
        htmlsource += '</div>';
        ...
        for (var i = 1; i <= doorCnt; i++) {
            ...
            htmlsource += '<div class="text-box" role="img" aria-label="컬러와 소재를 선택해 주세요.">';
    ...
    ClickPanel: function (data) {
        ...
        // step 1
        $("#" + dep.replace("/", "") + "> button.list-item__img").trigger('click');
        ...
        if ($('#btnStep1 > li.is-active').attr('data-dep') != 'F/P') {
            ...
            if ($prev_model_cnt == 1) {
                ...
                $("#btnStep1 > li[data-dep!='F/P'] > button").attr('title', '선택 가능');
            }
    ...
    Delete: function (obj) {
        ...
        messager.confirm(msg, "확인", "취소", function () {
            if ($("#sortable > div").length == 0) {
                ...
                $("#btnStep1 > li > button").attr('title', '선택 가능');
            }
        ...
        if (selectDepartment != "F/P") {
            if ($prev_model_cnt > 0) {
                ...
                if (cmbGb == 'A') {	 // 인피니트 1도어
                    ...
                    $("#btnStep1 > li[data-dep='IK/F'] > button").attr('title', '선택 가능');
                    $("#btnStep1 > li[data-dep='IF/S']").addClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='IF/S'] > button").attr('title', '선택 불가');
                    $("#btnStep1 > li[data-dep='F/S']").addClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='F/S'] > button").attr('title', '선택 불가');
                    $("#btnStep1 > li[data-dep='K/F']").addClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='K/F'] > button").attr('title', '선택 불가');
                }
                if (cmbGb == 'B') { // 프리스탠딩
                    $("#btnStep1 > li[data-dep='IK/F']").addClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='IK/F'] > button").attr('title', '선택 불가');
                    $("#btnStep1 > li[data-dep='IF/S']").removeClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='IF/S'] > button").attr('title', '선택 가능');
                    $("#btnStep1 > li[data-dep='F/S']").removeClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='F/S'] > button").attr('title', '선택 가능');
                    $("#btnStep1 > li[data-dep='K/F']").addClass(sDISABLED);
                    $("#btnStep1 > li[data-dep='K/F'] > button").attr('title', '선택 불가');
                }
                if (cmbGb == 'C') { // 키친핏
                    ...
                    $("#btnStep1 > li[data-dep='K/F'] > button").attr('title', '선택 가능');
                }
            }
        }
    ...
    SetSimulatorInfo: function () {
        ...
        if (selectDepartment == "F/P") {
            ...
            $("#bespokeInfo").show();
            $("#detail").attr("href", "/sec/" + goodspath);
        }
    },
    // acc click
    clickAccesary: function (data) {
        ...
        var pkgnm = data.dataset.pkgnm;
        var pkgalt ='';
        switch(data.dataset.pkgmdl){
	        case 'RA-F36CWSBA' : 
	        	pkgalt = '부피가 큰 식재료 수납이 용이한 빅박스, 온도 조절이 가능한 셰프 멀티 팬트리'; 
	        	break;
	        case 'RA-F36CWSBE' : 
	        	pkgalt = '싱싱함을 지켜주는 프레쉬 박스, 간편식 수납이 용이한 멀티 트레이, 온도 조절이 가능한 셰프 멀티 팬트리'; 
	        	break;
	        case 'RA-F36CWSCO' : 
	        	pkgalt = '싱싱함을 지켜주는 프레쉬 박스, 온도 조절이 가능한 셰프 멀티 팬트리'; 
	        	break;
	        case 'RA-F36CWSHW' : 
	        	pkgalt = '싱싱함을 지켜주는 프레쉬 박스, 간편식 수납이 용이한 멀티 트레이, 온도 조절이 가능한 셰프 멀티 팬트리'; 
	        	break;
	        case 'RA-F36CWSMV' : 
	        	pkgalt = '싱싱함을 지켜주는 프레쉬 박스, 부피가 큰 식재료 수납이 용이한 빅박스,온도 조절이 가능한 셰프 멀티 팬트리'; 
	        	break;
	        case 'RA-R23CWSBA' : 
	        	pkgalt = '부피가 큰 식재료 수납이 용이한 멀티 트레이, 온도 조절이 가능한 멀티 팬트리'; 
	        	break;
	        case 'RA-R23CWSBE' : 
	        	pkgalt = '간편식 수납이 용이한 멀티 트레이, 온도 조절이 가능한 멀티 팬트리'; 
	        	break;
	        case 'RA-R23CWSHW' : 
	        	pkgalt = '간편식 수납이 용이한 멀티 트레이, 온도 조절이 가능한 멀티 팬트리'; 
	        	break;
	        case 'RA-W24IAAWF' : 
	        	pkgalt = '부드러운 목재의 감성 우드 와인랙'; 
	        	break;
	        case 'RA-W24IAAWH' : 
	        	pkgalt = '깔끔한 사출 소재의 베이직 와인랙' ; 
	        	break;
	        default:
	        	pkgalt = '악세사리 설명' ; 
        		break;
                    ...
                    if (!loginFlag) {
                        if (loginYn == 'N') {
                            ...
                            messager.confirmCancel(msg, "확인", "취소", function () {
                                ...
                                window.location.href = "/sec/member/indexLogin/?returnUrl=" + window.location.pathname;
                            }, function () {
                                SaveData.bespokeMyModelAdd();
                            });
                            loginFlag = true;
...
function fncGetDataByStep3(data, dataStep) {
        ...
        for (var i = 0; i < filteredData.length; i++) {
            ...
            for (var j = 0; j < obj.length; j++) {
                ...
                if (toolTip.indexOf(objHeader1) > -1) {
                    html += '		<button class="btn-tooltip small" data-focus="opt_list_item_'+ j +'_1" onclick="event.preventDefault(); message.step3Pop(\'' + objHeader1 + '\', \'opt_list_item_'+ j +'_1\');"></button>';
                }
                html += '		</p>';
                html += '		<p class="text">';
                html += objHeader2;
                if (toolTip.indexOf(objHeader2) > -1) {
                    html += '			<button class="btn-tooltip small" data-focus="opt_list_item_'+ j +'_2" onclick="event.preventDefault(); message.step3Pop(\'' + objHeader2 + '\', \'opt_list_item_'+ j +'_2\');"></button>';
                }
                html += '		</p>';
                html += '		<p class="text">';
                html += objHeader3;
                if (toolTip.indexOf(objHeader3) > -1) {
                    html += '			<button class="btn-tooltip small" data-focus="opt_list_item_'+ j +'_3" onclick="event.preventDefault(); message.step3Pop(\'' + objHeader3 + '\', \'opt_list_item_'+ j +'_3\');"></button>';
                }
                html += '		</p>';
                html += '   </div>';
                ...
                $.each(pkg, function (k, data) {
                    ...
                    $.each(data, function (cnt, model) {
                        ...
                        if (cnt == 0 && rqidOpt == 'Y') {
                            ...
                            if (imgURL.indexOf("//") > -1) {
                                ...
                                subImgHtml += ' <img id="img_pkg_' + j + '_' + prdCnt + '" src="' + imgURL + '" alt="'+ pkgalt+'">';
                                subImgHtml += '</div>';
                            }
                    ...
                    if(k == "투명김치통 10.9L (3개 세트)"){
                        html += '   <button class="btn-tooltip small" data-focus="opt_list_item_clear3" onclick="event.preventDefault(); message.step3Pop(\'투명 김치통 (3개 세트)\', \'opt_list_item_clear3\');"></button>';
                    } else if(k == "투명김치통 10.9L (6개 세트)"){
                        html += '	<button class="btn-tooltip small" data-focus="opt_list_item_clear6" onclick="event.preventDefault(); message.step3Pop(\'투명 김치통 (6개 세트)\', \'opt_list_item_clear6\');"></button>';
                    } else if(k == "메탈김치통 11.2L (3개 세트)"){
                        html += '	<button class="btn-tooltip small" data-focus="opt_list_item_metal3" onclick="event.preventDefault(); message.step3Pop(\'메탈쿨링 김치통 (3개 세트)\', \'opt_list_item_metal3\');"></button>';
                    } else if(k == "메탈김치통 11.2L (1개 단품)"){
                        html += '   <button class="btn-tooltip small" data-focus="opt_list_item_metal1" onclick="event.preventDefault(); message.step3Pop(\'메탈쿨링 김치통 (1개 단품)\', \'opt_list_item_metal1\');"></button>';
                    }
                    html += '</p>';
    ...
    var $label = $("#step3_append_html").find("button.radio-box");
            ...
            $.each(hasColorList, function (k, itemList) {
                if (itemList.color.length > 0) {
                    ...
                    if(itemList.material != 'TOILETPAPER (글램 글래스)'){
                    	html += ' <button class="btn-tooltip small" aria-label="' + itemList.material + '의 특징을 설명하는 팝업을 출력하는 툴팁입니다." data-focus="refBespokePopColor'+ itemList.color[0].clrGrpId +'" onclick="event.preventDefault();message.step4Pop(' + itemList.color[0].clrGrpId + ')"></button>';
                    }
                    html += '</div>';
                    ...
                    $.each(itemList.color, function (m, prod) {
                        ...
                        html += '<button type="button" class="item-color">';
                        html += '<img src="' + prod.clrChipUrl + '" alt="'+prod.name["kr"]+'">';
                        html += '</button>';
            ...
            if (obj != null) {
                ...
                $.each(colorGrpIdArr, function (i, itemGrpList) {
                    if (itemGrpList.bspkColorTpCd == '2') {
                        ...
                        headHtml += '<button type="button" class="item-color">';
                        headHtml += '<img src="//images.samsung.com/is/image/samsung/p5/sec/mkt/bespokestudio/new-tab-' + imgUrlTxt + '.png?$ORIGIN_PNG$" alt="'+imgUrlTxt+'">';
                        headHtml += '</button>';
                        ...
                        $.each(obj, function (k, itemList) {
                            if (itemGrpList.clrGrpId == itemList.clrGrpId) {
                                prismColorIdArr.push('<button type="button" aria-label="' + itemList.name["kr"] + '" class="list-item" onclick="Cooler.SetPanelColor(this.dataset)"  data-clrGrpId="' + itemList.clrGrpId + '" data-colorNm="' + itemList.name["kr"] + '" data-colorData="' + itemList.name["en"] + '" style=" background: ' + itemList.name["en"] + '" data-bespokeYn="Y" ></button>');
                            }
                        });
                    }
                });
                for (var q = 0; q < 6; q++) {
                    for (var p = 0; p < 3; p++) {
                        subHtml += '<div class="prism-choice__list swiper-slide">';
                        for (var r = 0; r < 4; r++) {
                            for (var m = 0; m < 5; m++) {
                                var pCnt = 5 * p;
                                var rCnt = 15 * r;
...
//                html += '<div class="swiper-button-prev"></div>';
                html += '</div>';
                html += '<div class="prism-choice__btn-box">';
                html += '<button class="btn-underline" data-focus="allColorPop" onclick="message.allColor()">';
                ...
                $("#prism_360_color_detail > div").hide();
                $("#tab1").hide();
                ...
                $('#prism_360_color > li').click(function () {
                    ...
                    $("#prism_360_color_detail > div").hide();
                    $("#prism_360_color_detail > div > button[data-clrGrpId=" + $(this).attr('data-clrGrpId') + "]").parent().show();
                });
...
function PanelColorSetting(data) {
    ...
    $(door_id).attr("role", "img");
    $(door_id).attr("aria-label", colorNm);
    ...
    if (objFirst.length > 1) {
        ...
        if ($("#" + dataFirstInfo).attr('data-upDep') != 'F/P') {
            ...
            if (kitCbmGb.indexOf('A') > -1 || kitCbmGb.indexOf('C') > -1) {
                ...
                kitHtml += '               페어키트  ' + kitCd + '  ';
                kitHtml += '				<button class="btn-tooltip small" data-focus="pairKitPop" onclick="event.preventDefault(); message.pairKit();"></button>';
...
var SaveData = {
    ...
    SetModel: function (id, data) {
        var htmlSource = '';
        var refCnt = data.length;		// ref-count
    ...
    SetInfo: function (id, data) {
        ...
        htmlSource += '	<button class="btn-underline" data-focus="readMorePop_'+ id +'" onclick="event.preventDefault(); message.readMore(\''+id +'\',' + this.GetSpecificationData(data) + ');">';
    ...
    switch (parseInt(idx)) {
        case 0:
            if (department == "F/P") {
                ...
                $(step).find('.head-title .btn-tooltip').focus();
            } else {
                var step = '#step2_by_product .menu-step__head';
                $(step).next('.menu-step__contents').find('.img-item-list li:first-child button').focus();
            }
            break;
        case 1:
...
var bspkBuyAjax = function (data, data2, type) {
    ...
    if (data != '' && data2 != '') {
        ...
        var option = {
            url: "/sec/xhr/order/insertCartBespoke",
            dataType: "json",
            type: "POST",
            data: p_params,
