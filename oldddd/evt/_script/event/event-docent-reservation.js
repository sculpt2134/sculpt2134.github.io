	        ...
	        if(siDoCd == compareData){
	    		...
	    		if(selSidoPlazaIndex == 1){
	    			if (val.plazaNo == "162" && $(".store-alert").length == 0) {
	    				$(".store-name").after("<p class='store-alert'>※ 타임스퀘어 아트리움에서 진행되는 도슨트 투어는 방역패스 유효기간 기준에 따라 참여 가능합니다.</p>");
	    			} else if (val.plazaNo == "162" && $(".store-alert").length > 0) {
	    				$(".store-alert").remove();
	    				$(".store-name").after("<p class='store-alert'>※ 타임스퀘어 아트리움에서 진행되는 도슨트 투어는 방역패스 유효기간 기준에 따라 참여 가능합니다.</p>");
	    			} else if (val.plazaNo != "162") {
	    				$(".store-alert").remove();
	    			}
	...
	var options = {
		...
		,	done : function(result){
				...
				for(var i=dDay; i<7+dDay; i++){
					...
					var timeCd20Flag = false;
					var timeCd30Flag = false;
					var timeCd40Flag = false;
					var timeCd50Flag = false;
					var timeCd60Flag = false;
					var timeCd70Flag = false;
					...
					for(var j = 0; j < result.length; j++ ) {
						...
						if (timeCd10Flag == true && timeCd20Flag == true && timeCd30Flag == true && timeCd40Flag == true &&
								timeCd50Flag == true && timeCd60Flag == true && timeCd70Flag == true) { // 캠퍼스 베뉴가 아닐때
							calDateSetFlag = true;
							...
							if (visHopeDt === undefined) { // 달력선택값이
								if (dateToStringFormat(new Date(today),"yyyyMMdd") == result[j].APPL_COMPL_DATE) {
									if (activeDays.length == 0) {
										...
										if (timeCd20Flag) fnDisabledTimeCd(20);
										if (timeCd30Flag) fnDisabledTimeCd(30);
										if (timeCd40Flag) fnDisabledTimeCd(40);
										if (timeCd50Flag) fnDisabledTimeCd(50);
										if (timeCd60Flag) fnDisabledTimeCd(60);
										if (timeCd70Flag) fnDisabledTimeCd(70);
									}
								}
...
function getVisTimeCd () {
	...
	if (timeCd == '12:00~13:00') {
		return 10;
	} else if (timeCd == '13:00~14:00') {
		return 20;
	} else if (timeCd == '14:00~15:00') {
		return 30;
	} else if (timeCd == '15:00~16:00') {
		return 40;
	} else if (timeCd == '16:00~17:00') {
		return 50;
	} else if (timeCd == '17:00~18:00') {
		return 60;
	} else if (timeCd == '18:00~19:00') {
		return 70;
	}
}
...
function fnDisabledTimeCd (timeCd) {
	...
	if (timeCd == 10) {
		...
		$docentTimePos = $docentTimePos.eq(3);
		...
		$docentTimePos = $docentTimePos.eq(4);
		...
		$docentTimePos = $docentTimePos.eq(5);
		...
		$docentTimePos = $docentTimePos.eq(6);
